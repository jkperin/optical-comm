...
lambp = SpectrumData(:, 1);
Signal = Channels(wavelength.'*1e-9, dBm2Watt(Pin.' + offset), 'forward');

leg = {};
for k = 1:length(PpumpmW)
    
    [pks, locs] = findpeaks(SpectrumData(:, k+1)); 
    threshold = max(pks)-15;
    locs = locs(pks > threshold);
    pks = pks(pks > threshold);
    
    figure(101), hold on, box on
    plot(lambp, SpectrumData(:, k+1))
    plot(lambp(locs), pks, 'sr', 'MarkerSize', 4)
    xlabel('Wavelength (nm)')
    ylabel('Power (dBm)')
    drawnow 
   
    Pp = dBm2Watt(pks.');
    Pp = PumpPowermW(k)*1e-3*Pp/sum(Pp);
    Pump = Channels(1e-9*lambp(locs).', Pp, 'forward');
...
%     [GaindB, Pump_out, Psignal_out, Pase, sol] = E.SHB(Pump, Signal, ASEf, ASEb, BWref, 'three-level', 100, false);
    [GaindB, Pump_out, Psignal_out, Pase, sol] = E.propagate(Pump, Signal, ASEf, ASEb, BWref, 'two-level', 50, false);
