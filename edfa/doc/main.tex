...
For the ON channels the gain is relatively high, since $g(\lambda_k) \geq e^{\alpha_{SMF}(\lambda_k)l}$. Thus, we can use the approximation $\frac{g(\lambda_k)}{g(\lambda_k)-1} \approx 1$, which results in 
\begin{equation}
	C =\sum_{k = 1}^{K} \Delta f\log_2\bigg(1 + \frac{P_k}{2(N-1)\Delta fn_{sp}(\lambda_k)h\nu_k}\bigg) 
\end{equation}

The summation is performed over the channels that have gain higher than the span attenuation:

\begin{equation}
SE =\sum_{k} \bm{1}\{g(\lambda_k) \geq e^{\alpha_{SMF}(\lambda_k)l}\}\log_2\bigg(1 + \frac{P_k}{2(N-1)\Delta fn_{sp}(\lambda_k)h\nu_k}\bigg) 
\end{equation}
