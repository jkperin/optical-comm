%% Calculate optical power penalty (PP) as a function of the modulator 
%% cutoff frequency (f3dB) without quantization. The clipping ratio at the 
%% transmitter was optimize for each f3dB using the function dc_ofdm_ber_vs_clipping_ratio.m
% clear, clc, close all
...
%         RCLIP = [4.2+0.1	4+0.1	3.7+0.1	3.7+0.1	3.7	3.7	3.7	3.7	3.7];       % optmized clipping ratio
%         RCLIP = [4.2000 4.0000 3.9000 3.7000 3.7000 3.7000 3.7000 3.7000 3.7000];
        RCLIP = [4.2 4.0 3.9 3.7 3.7 3.7 3.7 3.7 3.7];
...
%         RCLIP = [4.5	4.5	4.5	4.5	4.5	4.5	4.5	4.5	4.5]+0.1;       % optmized clipping ratio
%         RCLIP = [4.6000 4.3000 4.2000 4.1000 4.1000 4.0000 4.0000 4.0000 3.9000];
        RCLIP = [4.3 4.2 4.1 4.0 4.0 4.0 4.0 4.0 4.0];
...
%         RCLIP = [3.9+0.1	3.7+0.1	3.7	3.7	3.7	3.7	3.7	3.7	3.7];       % optmized clipping ratio
%         RCLIP = [3.9000 3.8000 3.7000 3.7000 3.7000 3.7000 3.7000 3.7000 3.7000];
        RCLIP = [3.9 3.8 3.7 3.7 3.7 3.7 3.7 3.7 3.7];
...
%         RCLIP =  [4.6000 4.1000 4.0000 4.0000 3.9000 4.0000 4.0000 3.9000 3.9000];
        RCLIP = [4.4 4.3 4.1 4.1 3.9 3.9 3.9 3.9 3.9];
...
if ofdm.full_dc || exist('ofdm.full_dc', 'var') == 0
    fprintf('------ %s, CS = %d, full DC ------\n', sim.type, ofdm.CS)
else
    fprintf('------ %s, CS = %d, reduced DC ------\n', sim.type, ofdm.CS)
end
...
plot(Fnl/1e9, berint.', 'xk', 'MarkerSize', 6)
xlabel('Cut-off frequency (GHz)')
ylabel('log_{10}(BER)')
legend('BER counted', 'BER estimaded', 'Target BER', '95%% confidence intervals')
...
if sim.save
    ...
    if sim.quantiz
        ...
        filename = [sim.type '_CS=' num2str(ofdm.CS)];
    end
