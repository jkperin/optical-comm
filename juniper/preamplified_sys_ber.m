...
for k = 1:length(Ptx)
    ...
    if isfield(sim, 'coherentDetector') && sim.coherentDetector
        disp('Assuming detection is done using a coherent detector')
        [ber.count(k), OSNRdB(k)] = ber_preamp_sys_montecarlo_coherent_rx(mpam, Tx, Fibers, Amp, Rx, sim)
    else
...
if sim.shouldPlot('BER') && length(ber.count) > 1
    if isfield(sim, 'coherentDetector') && sim.coherentDetector
        ...
        hline = plot(OSNRdB, log10(ber.count), '-o', 'LineWidth', 2);
        legend('Counted')
        set(gca, 'FontSize', 12)
