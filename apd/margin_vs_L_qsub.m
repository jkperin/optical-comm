function margin_vs_L_qsub(M, ka, level_spacing, BW0GHz, GainBWGHz, modBWGHz)
%% Function to calculate power margin vs fiber length in bactch simulation on corn
...
MargindB = zeros(length(L), length(Gains));
for l = 1:length(L)
    link = fiber(L(l)); % no attenuation, SSMF
...
%     drawnow
    ...
    % Calculate margin   
    MargindB(l, :) = PtxdBm_pin_BERtarget - PtxdBm_BERtarget;

    % Plot Gain x Margin
%     figure(1000+l), hold on, box on, grid on
%     plot(Gains, MargindB(l, :))
%     plot(Gopt_margin(l), OptMargindB(l), 'o') 
%     title(sprintf('L = %d', link.L/1e3))
end

% figure
% plot(L/1e3, OptMargindB)
% xlabel('Fiber Length (km)')
% ylabel('Margin (dB)')
% 
% figure
% plot(L/1e3, Gopt_margin)
% xlabel('Fiber Length (km)')
% ylabel('Gain')
% 
...
% Save results
% Save results
filename = sprintf('results/data/margin_vs_L_%d-PAM_%s_ka=%d_BW0=%d_GainBW=%d_modBW=%d',...
