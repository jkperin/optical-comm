...
\section{ISI enhanced signal-dependent noise}

Fig. \ref{fig:eq_discrete_time} shows the equivalent discrete-time system model, where $x[n]$ are the transmitted symbols, $p[n]$ corresponds to the pulse shape and includes any other frequency response limitations such as modulator and fiber.  $s[n]$ is the signal dependent noise whose variance is determined by $x[n]\ast g[n]$. Note, that $s[n]$ is signal-dependent only for simplicity. The effects of signal-independent noises can be added later by superposition. $h[n]$ includes the other frequency response limitations such as the APD frequency response. Moreover, $h[n]$ includes the matched filter and the linear equalizer.
...
If $x[n]\ast g[n]$ is ISI free, then the noise samples $s[n]$ will be uncorrelated and the system penalty will be due only to noise enhancement penalty. When $x[n]\ast p[n]$ is not ISI free there will be an additional penalty caused by the ISI enhanced signal-dependent noise.
