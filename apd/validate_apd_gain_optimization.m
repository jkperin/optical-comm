...
% apd_opt = apd(10.0851, 0.09, 340e11, 1, 10e-9); % uniform, infinite gain x BW product
% apd_opt = apd_gain_optmization(mpam, tx, apd_opt, rx, sim);

apd_opt = apd(12.7365, 0.09, 340e11, 1, 10e-9); % nonuniform, infinite gain x BW product
apd_opt.optimize_gain(mpam, tx, rx, sim);

%
GainsdB = [4:13 apd_opt.GaindB 20];
...
for k= 1:length(GainsdB)
    ...
    apdG = apd(GainsdB(k), 0.09, 340e11, 1, 10e-9);
