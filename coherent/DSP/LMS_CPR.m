...
function [xhat, xD, eps, epsD, WT, thetahat] = LMS_CPR(x, tsamp, Ysq, Nsymb, oversamp, AdEq,Cpr , PN, snr_dB)
    ...
    for k = 1:Nsymb;
        ...
        if (k>=L)
            ...
            % based on transmitted symbols
            eps(:,ind) = xhat(:,ind) - x(:,ind);

            % based on decision symbols
            epsD(:,ind) = xhat(:,ind) - xD(:,ind);
        end
        
        
        % update WT =======================================================
        if mod(k-L,Delta+1) == 0
            ind = k-Delta;
            ...
            epsk = decwtvec(k)*eps(:,ind)+~decwtvec(k)*epsD(:,ind);
            % update WT
