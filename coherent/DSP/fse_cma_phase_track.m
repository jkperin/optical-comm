function [Y, Wx, Wy, MSE] = fse_cma_phase_track(eq, X, Ytrain, M)
...
for n = ((kstart-1)/ros+1):Nsymb-ceil(Ntaps/2) % n runs over symbols
    ...
    Wx(:, filt) = Wx(:, filt) + mu*ex(n)*yx_cma*z';
    Wy(:, filt) = Wy(:, filt) + mu*ey(n)*yy_cma*z';
    ...
    % Phase tracker
    yx_hat(n) = yx_cma*exp(1j*phix);
    yy_hat(n) = yy_cma*exp(1j*phiy);
    
    if n < Ntrain % Training based on training sequence
        ephix = yx_hat(n) - Ytrain(1, n);
        ephiy = yx_hat(n) - Ytrain(2, n);
    else % based on deteced symbols
        ephix = yx_hat(n) - qammod(qamdemod(yx_cma, M, 0, 'Gray'), M, 0, 'Gray');
        ephiy = yx_hat(n) - qammod(qamdemod(yy_cma, M, 0, 'Gray'), M, 0, 'Gray');
    end
    
    phix = phix - mu*imag(yx_hat(n)*conj(ephix));
    phiy = phiy - mu*imag(yy_hat(n)*conj(ephiy));
end
