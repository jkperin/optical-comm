...
\begin{equation}
G(z) = \frac{z^2(\xi\omega_nT_s + T_s^2\omega_n^2/4) + }{z^2 - 2z + 1}
\end{equation}
...
I_q(t) = \frac{R}{p}\sqrt{P_{LO}}(\sqrt{P_{rx}}y_{s,q}(t) +  E_{sp}(t)) + I_{sh,q}
\end{align}
for the in-phase and quadrature components in each polarization. $E_{sp}(t)$ is the amplified spontaneous emission (ASE) noise, and $I_{sh,i}$ is the noise component due to shot noise.

The transmitted electric field for each polarization is given by
\begin{equation}
E_{tx} = \sqrt{P_{rx}/p}(y_{s,i}(t) + y_{s,q}(t))
\end{equation}
This implies that since $E(|E_{tx}|^2) = P_{rx}/p$, then $E(|y_{s,i}(t) + y_{s,q}(t)|^2) = 2E(|y_{s,i}(t)|^2 = 2E(|y_{s,q}(t)|^2 = 1$ i.e., $E(|y_{s,q}(t)|^2 = E(|y_{s,i}(t)|^2 = 1/2$.

Therefore, the signal power per real dimension is
\begin{equation}
P_{sig} = \frac{P_{LO}P_{rx}}{2p^2}
\end{equation}
