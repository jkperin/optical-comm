...
% ---------- Parameter Range -----------
mu_CMA = 0.0015:.0005:.01;
mu_PT = 0.004:.001:.006;
LFilt = [6];
% --------- Performance Metric ---------
MSE_meas = zeros(length(mu_CMA),length(mu_PT),length(LFilt));
...
for m1=1:length(mu_CMA) 
    for m2=1:length(mu_PT)
        for l=1:length(LFilt)
            ...
            MSE_meas(m1,m2,l) = mean(sqerr(measind));
            ...
            Qmeas(m1,m2,l) = (Q1imeas+Q1qmeas+Q2imeas+Q2qmeas)/4;
            % Bit-error ratio, average over all four tributaries, starting at 'startmeas', computed based on measured Q factors
            BER_from_Qmeas(m1,m2,l) = (0.5*erfc(Q1imeas/sqrt(2))+0.5*erfc(Q1qmeas/sqrt(2))+...
            ...
            BERmeas(m1,m2,l) = num_bit_err_meas/4/numsymbmeas;
